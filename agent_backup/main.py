    ...
    for repo_name in repos_to_scan:
        ...
        try:
            ...
            if os.path.exists(constants.WORKSPACE_DIR):
                # Windows-friendly directory removal with error handling
                try:
                    def handle_remove_readonly(func, path, exc):
                        """Error handler for shutil.rmtree on Windows."""
                        # Try to change file permissions and remove read-only flag
                        try:
                        ...
                        try:
                            ...
                            # If still failing, ignore it (file might be locked by another process)
                            pass
                    
                    # Use the error handler for Windows
                    if sys.platform == 'win32':
                        shutil.rmtree(constants.WORKSPACE_DIR, onerror=handle_remove_readonly)
                    else:
                        # On Unix-like systems, use standard removal
                        shutil.rmtree(constants.WORKSPACE_DIR)
                    print("--- 🧹 Workspace cleaned ---")
                except Exception as e:
                    # If cleanup fails, log it but don't crash
                    print(f"⚠️ (Main): Warning - could not fully clean workspace: {e}")
                    print("--- 🧹 Workspace cleanup attempted ---")
